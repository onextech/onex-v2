...
import { MOCK_GROUP_PRESS_RELEASES } from '@onex/mocks'
import { GetStaticPaths, GetStaticProps } from 'next'
import { PressRelease } from '@onex/types'

export const getStaticPaths: GetStaticPaths = async () => {
  return {
    paths: MOCK_GROUP_PRESS_RELEASES.map(({ slug }) => ({
      params: { slug },
    })),
    fallback: false,
  }
...
export const getStaticProps: GetStaticProps = (context) => {
  const pressRelease = MOCK_GROUP_PRESS_RELEASES.find(
    ({ slug }) => slug === context.params.slug
  )
  const otherPressReleases = MOCK_GROUP_PRESS_RELEASES.filter(
    ({ slug }) => slug !== context.params.slug
  ).slice(0, 3)
